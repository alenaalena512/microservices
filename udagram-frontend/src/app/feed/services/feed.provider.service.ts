...
export class FeedProviderService {
  ...
  async getFeed(): Promise<BehaviorSubject<FeedItem[]>> {
    const req = await this.api.get(environment.feedApiHost, '/feed');
  ...
  async uploadFeedItem(caption: string, file: File): Promise<any> {
    const res = await this.api.upload(environment.feedApiHost, '/feed', file, {caption: caption, url: file.name});
